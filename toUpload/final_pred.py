...
# create the arduino serial port 
# ArduinoUnoSerial = serial.Serial('com12',9600)
...
class Application:
    ...
    def __init__(self):
        ...
        self.d = {"English":"en", "Kanada":"kn", "Hindi":"hi", "Marathi":"mr", "French":"fr", "German":"es", "Spanish":"sp", "Korian":"ko", "Arebic":"ar","Gujrati":"gu"}
        self.slang=""
        ...
        self.root.configure(bg="MistyRose")
        # self.root.backGroundImage=PhotoImage(file="bg1.jpeg")
        # self.root.backGroundImageLabel=Label(self.root,image=self.backGroundImage)
        # self.root.backGroundImageLabel.place(x=0,y=0)
        # self.root.bg = PhotoImage(file = "bg1.jpeg")
        # self.bg = PhotoImage(file = "bg1.jpeg")

        self.panel = tk.Label(self.root)
        self.panel.place(x=170, y=90, width=500, height=450)
        self.panel.configure(bg="MistyRose")

        self.panel2 = tk.Label(self.root)  # initialize image panel
        self.panel2.place(x=870, y=90, width=500, height=450)

        self.T = tk.Label(self.root)
        self.T.place(x=750, y=5)
        self.T.config(text="Gesto-Sign", font=("Georgia", 30, "bold"))
        self.T.configure(bg="MistyRose")
        
# ccharacter
        self.T1 = tk.Label(self.root)
        self.T1.place(x=10, y=580)
        self.T1.config(text="Character :", font=("Georgia", 20, "bold"))
        self.T1.configure(bg="MistyRose")

        self.panel3 = tk.Label(self.root)  # Current Symbol
        self.panel3.place(x=180, y=575)
        self.panel3.config(font=("Georgia", 10 ))
        self.panel3.configure(bg="MistyRose")

# Sentence
        
        self.panel5 = tk.Label(self.root)  # Current Sentence
        self.panel5.place(x=180, y=625)
        self.panel5.config(font=("Georgia", 10 ))
        self.panel5.configure(bg="MistyRose")

        self.T3 = tk.Label(self.root)
        self.T3.place(x=10, y=632)
        self.T3.config(text="Sentence :", font=("Georgia", 20, "bold"))
        self.T3.configure(bg="MistyRose")

# Sugessions ans Buttons 
        
        self.T4 = tk.Label(self.root)
        self.T4.place(x=10, y=700)
        self.T4.config(text="Suggestions :", font=("Georgia", 20, "bold"))
        self.T4.configure(bg="MistyRose")


        self.b1=tk.Button(self.root)
        self.b1.config(font=("Georgia", 20))
        self.b1.place(x=200,y=700)
        self.b1.configure(bg="MistyRose")

        self.b2 = tk.Button(self.root)
        self.b2.config(font=("Georgia", 20))
        self.b2.place(x=350, y=700)
        self.b2.configure(bg="MistyRose")

        self.b3 = tk.Button(self.root)
        self.b3.config(font=("Georgia", 20))
        self.b3.place(x=500, y=700)
        self.b3.configure(bg="MistyRose")

        self.b4 = tk.Button(self.root)
        self.b4.config(font=("Georgia", 20))
        self.b4.place(x=650, y=700)
        self.b4.configure(bg="MistyRose")

# OPerating Buttons

        self.speak = tk.Button(self.root)
        self.speak.place(x=950, y=710)
        self.speak.config(text="Speak", font=("Georgia", 20), wraplength=100, command=self.speak_mlang)

        self.speak = tk.Button(self.root)
        self.speak.place(x=1130, y=710)
        self.speak.config(text="IOT", font=("Georgia", 20), wraplength=100, command=self.iot_operate)

        self.clear = tk.Button(self.root)
        self.clear.place(x=1285, y=710)
        self.clear.config(text="Clear", font=("Georgia", 20), wraplength=100, command=self.clear_fun)
...
# Language Selection
        ...
        self.L1.place(x=950, y=610)
        self.L1.config(text="Select Languages :", font=("Georgia", 15, "bold"))
        self.L1.configure(bg="MistyRose")



        self.combo_box = ttk.Combobox(self.root,values=list(self.d.keys()))
        self.combo_box.pack(pady=15)
        # self.n = tk.StringVar()
        # self.combo_box = ttk.Combobox(self.root, values=self.d.keys())
        # self.combo_box.pack(pady=15)
        # self.combo_box.configure(command=self.combobox_callback)
        self.combo_box.place(x=1150,y=609)
        # self.combo_box.bind("<<ComboboxSelected>>", self.option_selected)
        # self.combo_box.configure(command=option_selected(self.root,))
    ...
    def video_loop(self):
        try:
            ...
            if hands:
                ...
                # white = cv2.imread("E:\Sign-Language-To-Text-and-Speech-Conversion-master\white.jpg")
                white = cv2.imread("E:\Sign-Language-To-Text-and-Speech-Conversion-master\white.jpg")
                ...
                if handz:
                    ...
                    self.panel3.config(text=self.current_symbol, font=("Georgia", 30))

                    #self.panel4.config(text=self.word, font=("Georgia", 30))



                    self.b1.config(text=self.word1, font=("Georgia", 20), wraplength=825, command=self.action1)
                    self.b2.config(text=self.word2, font=("Georgia", 20), wraplength=825,  command=self.action2)
                    self.b3.config(text=self.word3, font=("Georgia", 20), wraplength=825,  command=self.action3)
                    self.b4.config(text=self.word4, font=("Georgia", 20), wraplength=825,  command=self.action4)

            self.panel5.config(text=self.str, font=("Georgia", 30), wraplength=1025)
        except Exception:
...
# Multilanguage Transcription
    ...
    def speak_mlang(self):
        translator = Translator()

        # Assuming 'str' is the text you want to translate
        text_to_translate = self.str  # Replace 'str' with the actual text you want to translate
        # text_to_translate = "Hello"

        self.selected_language = self.combo_box.get()

        print(self.selected_language)
        translated = translator.translate(text_to_translate, src='en', dest=self.d[self.selected_language])
        translated_text = translated.text

# Speak the translated text
        tts = gTTS(text=translated_text, lang=self.d[self.selected_language])  # 'mr' is the language code for Marathi, change it to the desired language code
    ...
    def iot_operate(self):
        signal = self.str.lower()
        signal = signal.strip()
        # print(signal)
...
#  on and off fan 
        if(signal=="on f"):                                                                                                                                                                                                                            
            ArduinoUnoSerial.write(bytes('3','utf-8')) 
            print ("LED turned ON")           
            time.sleep(1)
        if(signal=="off f"):                                                                                                                                                                                                                            
            ArduinoUnoSerial.write(bytes('4','utf-8')) 
            print ("LED turned ON")           
            time.sleep(1)
